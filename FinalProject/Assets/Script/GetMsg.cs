...
public class GetMsg : MonoBehaviour
{
    ...
    void Local(Pos _msg)
    {
        ...
        {
            //float.Parse(y[y.Length - 1]) * 100+500
            CarCamera.transform.position = new Vector3(float.Parse(y[y.Length - 1])*100*-1+500  , 500, float.Parse(x[x.Length - 1])*100+500 );
            
            CarCamera.transform.rotation = new Quaternion(float.Parse(ry[ry.Length - 1]), float.Parse(rz[rz.Length - 1])*-1 , float.Parse(rx[rx.Length - 1]) , float.Parse(rw[rw.Length - 1]));
        }
