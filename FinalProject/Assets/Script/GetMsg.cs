...
public class GetMsg : MonoBehaviour
{
    ...
    void Local(Pos _msg)
    {
        ...
        if (new Vector3(float.Parse(x[x.Length - 1]), float.Parse(y[y.Length - 1]), float.Parse(z[z.Length - 1])) != new Vector3(0, 0, 0))
        {
            ...
            CarCamera.transform.position = new Vector3(float.Parse(x[x.Length - 1])*10+500  , 500.15f, float.Parse(y[y.Length - 1])*10+500 );
            
            CarCamera.transform.rotation = new Quaternion(0, float.Parse(rz[rz.Length - 1])*-1 , 0 , float.Parse(rw[rw.Length - 1]));
        }
